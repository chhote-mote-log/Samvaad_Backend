...
export async function startVideoModerationPipeline() {
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      ) as VideoModerationMessage;
      console.log(
        `🎥 Received video moderation request for user: ${raw.userId}`
      );
      ...
      try {
        ...
        const response = await axios.get(raw.videoUrl, {
          responseType: "stream",
        });
        ...
        await execAsync(
          `ffmpeg -y -i ${videoPath} -vn -acodec pcm_s16le -ar 44100 -ac 2 ${audioPath}`
        );
